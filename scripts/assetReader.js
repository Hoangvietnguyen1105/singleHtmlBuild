...
// read and write all animation
console.log("--------------- ANIMATION ---------------");
var allAnimation = read("animations");
allAnimation.forEach((animation, index) => {
  fs.appendFileSync(OUTPUT_PATH, JSON.stringify(animation) + ",\n");
  console.log("Writing model: " + animation.key + " - " + animation.size.toFixed(2) + " KB");
  if (index === allAnimation.length - 1) {
    console.log("All animations passed: " + allAnimation.length);
    totalAnimationSize = allAnimation.reduce((total, animation) => total + animation.size, 0);
  }
});
...
totalSize = totalSpriteSize + totalTextureSize + totalFontSize + totalModelSize + totalAnimationSize + totalSpineSize + totalAudioSize;
