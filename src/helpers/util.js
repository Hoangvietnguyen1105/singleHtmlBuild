...
export class Util {
  ...
  static randomEven(min, max) {
    let randomNum = Math.floor(Math.random() * (max - min + 1) + min);
    if (randomNum % 2 !== 0) {
        randomNum += 1; // Nếu số là lẻ, tăng lên 1 để trở thành số chẵn
    }
    return randomNum;
  }
  static randomOdd(min, max) {
    let randomNum = Math.floor(Math.random() * (max - min + 1) + min);
    if (randomNum % 2 === 0) {
        randomNum += 1; // Nếu số là chẵn, tăng lên 1 để trở thành số lẻ
  ...
  static getRandomValue() {
    // Sử dụng Math.random() để tạo số ngẫu nhiên từ 0 đến 1
    const randomValue = Math.random();

    // Tạo danh sách các giá trị có thể
    const possibleValues = [22.5, 45, 67.5, 90];

    // Chọn ngẫu nhiên một phần tử từ danh sách possibleValues
    const randomIndex = Math.floor(randomValue * possibleValues.length);
    return possibleValues[randomIndex];
  }

  static deleteElement(array, elementToDelete) {
    let indexToDelete = array.indexOf(elementToDelete);
